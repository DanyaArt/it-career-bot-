...
def universities_management(message):
    ...
    bot.reply_to(message, "🎓 Управление вузами\n\nВыберите действие:", reply_markup=markup)
...
def add_university_process(message):
    ...
    if step == 0:
        ...
        try:
            specializations = db.get_specialization_names()
            if not specializations:
                bot.reply_to(message, "❌ Нет доступных специализаций в базе данных")
                return
            
            # Показываем кнопки специализаций
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
            
            # Добавляем кнопки специализаций
            for i in range(0, len(specializations), 2):
                if i + 1 < len(specializations):
                    markup.add(
                        types.KeyboardButton(specializations[i]),
                        types.KeyboardButton(specializations[i + 1])
                    )
                else:
                    markup.add(types.KeyboardButton(specializations[i]))
            
            markup.add(types.KeyboardButton('⬅️ Отмена'))
            
            bot.reply_to(message, 
                         f"📝 Вуз: {message.text}\n\n"
                         f"Выберите специализацию:",
                         reply_markup=markup)
        except Exception as e:
            bot.reply_to(message, f"❌ Ошибка при получении специализаций: {e}")
            return
                     
    elif step == 1:
        # Проверяем, что выбранная специализация существует в БД
        try:
            specializations = db.get_specialization_names()
            if message.text not in specializations:
                bot.reply_to(message, "❌ Выберите специализацию из кнопок")
                return
            
            specialization_name = message.text
        except Exception as e:
            bot.reply_to(message, f"❌ Ошибка при проверке специализации: {e}")
            return
        
        try:
            specialization_id = db.get_specialization_id_by_name(specialization_name)
            if not specialization_id:
                bot.reply_to(message, "❌ Специализация не найдена")
                return
            
            state['specialization_id'] = specialization_id
            state['specialization_name'] = specialization_name
            state['step'] = 2
            ...
            bot.reply_to(message, 
                         ...
                         f"🎓 Специализация: {specialization_name}\n\n"
                         f"📍 Отправьте город вуза:",
                         reply_markup=markup)
                         
        except Exception as e:
            bot.reply_to(message, f"❌ Ошибка при выборе специализации: {e}")
        ...
        # Сохраняем сайт (опционально)
        url = message.text if message.text != 'Пропустить' else None
        state['url'] = url
        state['step'] = 4
        ...
        bot.reply_to(message, 
                     ...
                     f"📊 Отправьте минимальные баллы ЕГЭ (например: 200):",
                     reply_markup=markup)
                     
    elif step == 4:
        # Сохраняем минимальные баллы
        try:
            score_min = int(message.text.strip())
            if score_min < 0 or score_min > 400:
                bot.reply_to(message, "❌ Минимальные баллы должны быть от 0 до 400")
                return
            ...
            bot.reply_to(message, 
                         ...
                         f"🎓 Специализация: {state['specialization_name']}\n"
                         f"📍 Город: {state['city']}\n"
                         f"🌐 Сайт: {state['url'] or 'Не указан'}\n"
                         f"📊 Мин. баллы: {score_min}\n\n"
                         f"📈 Отправьте максимальные баллы ЕГЭ (например: 250):",
                         reply_markup=markup,
                         disable_web_page_preview=True)
                         
        except ValueError:
            bot.reply_to(message, "❌ Введите корректное число для минимальных баллов")
            return
            
    elif step == 5:
        # Сохраняем максимальные баллы и добавляем вуз
        try:
            score_max = int(message.text.strip())
            if score_max < state['score_min'] or score_max > 400:
                bot.reply_to(message, f"❌ Максимальные баллы должны быть от {state['score_min']} до 400")
                return
            
            try:
                # Добавляем вуз в базу с указанными баллами
                db.add_university(
                ...
                # Синхронизация сайта после добавления
                try:
                    db.sync_website_data()
                    bot.reply_to(message, 
                                f"✅ Вуз успешно добавлен!\n\n"
                                f"📝 Вуз: {state['university_name']}\n"
                                f"🎓 Специализация: {state['specialization_name']}\n"
                                f"📍 Город: {state['city']}\n"
                                f"📊 Баллы: {state['score_min']}-{score_max}\n"
                                f"🌐 Сайт: {state['url'] or 'Не указан'}",
                                disable_web_page_preview=True)
                except Exception as e:
                    bot.reply_to(message, f"✅ Вуз добавлен, но ошибка обновления сайта: {e}")
                del admin_states[user_id]
                admin_panel(message)
                
            except Exception as e:
                bot.reply_to(message, f"❌ Ошибка при добавлении вуза: {e}")
                
        except ValueError:
            bot.reply_to(message, "❌ Введите корректное число для максимальных баллов")
            return
